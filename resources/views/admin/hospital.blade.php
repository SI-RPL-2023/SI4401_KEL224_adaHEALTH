              ...
              <tbody>
                @forelse ($hospitals as $hospital)
                  <tr class="{{ $loop->iteration % 2 == 0 ? 'bg-gray-100' : 'bg-white' }}">
                    ...
                    <td class="py-3 px-4">{{ $hospital->address }}</td>
                    <td class="py-3 px-4"><img src="{{ url('images/'.$hospital->images) }}" alt="{{ $hospital->name }}" class="w-20 rounded-lg"></td>
    ...
    <div id="hospital-form" class="hidden">
        <form action="{{ route('store.hospital') }}" class="pt-20" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="border rounded-lg py-2 px-3 w-full @error('name') border-red-500 @enderror" value="{{ old('name') }}" placeholder="Enter hospital name">
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" cols="30" rows="10" class="border rounded-lg py-2 px-3 w-full @error('description') border-red-500 @enderror" placeholder="Enter hospital description">{{ old('description') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <textarea name="address" id="address" cols="30" rows="10" class="border rounded-lg py-2 px-3 w-full @error('address') border-red-500 @enderror" placeholder="Enter hospital address">{{ old('address') }}</textarea>
