                ...
                <div>
                    ...
                    @else
                        @if (Auth::check() && (empty(Auth::user()->name) || empty(Auth::user()->date_birth) || empty(Auth::user()->gender) || empty(Auth::user()->address)))
                            <p class="mt-10">Lengkapi profil Anda terlebih dahulu untuk dapat melakukan pemesanan.</p>
                            <p></p>
                            <button><a href="{{ route('profile.show', ['id' => Auth::user()->id]) }}" class="text-primary">Lengkapi Disini</a></button>
                        @else
